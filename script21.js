...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function showHint() {
    ...
    // ✅ Get the current level correctly
    const level = localStorage.getItem('selectedGameLevel') || gameLevel || 'Hard';

    // ✅ Only block Hint for God level
    if (level === 'God') {
      statusMessage.textContent = "No hints in God mode.";
      return;
    }
    ...
    // Check if opponent (X) could win next
    for (let idx of emptyCells) {
      board[idx] = 'X';
      if (checkWin(board, 'X')) {
        board[idx] = null;
        highlightHintCell(idx);
        statusMessage.textContent = "Opponent could win here!";
        return;
      }
      board[idx] = null;
    }

    // Check if you (O) could win
    for (let idx of emptyCells) {
      board[idx] = 'O';
      if (checkWin(board, 'O')) {
        board[idx] = null;
        highlightHintCell(idx);
        statusMessage.textContent = "Try this move!";
        return;
      }
      board[idx] = null;
    }
...
function undoMove() {
  ...
  // ❌ Undo केवल Easy और Medium में allowed है
  if (level !== 'Easy' && level !== 'Medium') {
    statusMessage.textContent = 'Undo not allowed in this mode.';
    return;
  }

  if (!Array.isArray(moveHistory) || moveHistory.length < 2 || gameOver) return;

  // AI move undo
  const lastAIMove = moveHistory.pop();
  if (lastAIMove) {
    board[lastAIMove.index] = null;
    const cell = gameBoard.querySelector(`[data-index="${lastAIMove.index}"]`);
    if (cell) {
      cell.textContent = '';
      cell.classList.remove('x', 'o', 'last-move-player', 'last-move-ai');
    }
  }
  ...
  if (lastPlayerMove) {
    board[lastPlayerMove.index] = null;
    const cell = gameBoard.querySelector(`[data-index="${lastPlayerMove.index}"]`);
    if (cell) {
      cell.textContent = '';
      cell.classList.remove('x', 'o', 'last-move-player', 'last-move-ai');
    }
...
// ✅ Ensure selectedGameLevel is present, fallback to 'Hard'
if (!localStorage.getItem('selectedGameLevel')) {
  localStorage.setItem('selectedGameLevel', gameLevel);  // gameLevel yahi 'Hard' hai
}
...
function makeMove(index, player) {
  if (!board[index]) {
    ...
    // ✅ सभी levels में moveHistory अपडेट करें
    if (!Array.isArray(moveHistory)) moveHistory = [];
    moveHistory.push({ index, player });

    // 🎯 सिर्फ God level के लिए animation/fade effect
    const gameLevel = localStorage.getItem('selectedGameLevel');
    if (gameLevel === 'God') {
